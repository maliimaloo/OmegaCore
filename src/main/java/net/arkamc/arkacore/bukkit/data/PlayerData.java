...
import net.arkamc.arkacore.bukkit.util.model.IFinancialCallback;
...
/**
* Représente les données d'un joueur dans le plugin ArkaCore.
 */
@AllArgsConstructor
public final class PlayerData {
    ...
    private String jedisAccountKey = "arkacore:account:{unique_id}";
    private String jedisLogsKey = "arkacore:credit_logs:{unique_id}";
    ...
    public Boolean isWhisper() {
        return Boolean.getBoolean(this.getHashValue("whisper"));
    }

    public void setWhisper(Boolean enabled) {
        this.setHashValue("whisper", Boolean.toString(enabled));
    }
    ...
    /**
     * Récupère le nombre de crédits du joueur.
     *
     * @return Le nombre de crédits du joueur.
     */
    public long getCredit() {
        return Long.parseLong(this.getHashValue("credit"));
    }

    /**
     * Crédite le compte du joueur avec une certaine quantité de crédits.
     *
    ...
    /**
     * Retire une certaine quantité de credit du compte du joueur.
     *
    ...
    /**
     * Vérifie si le joueur possède suffisamment de crédits.
     *
     * @param amount Le montant a vérifié
     * @return Vrai si le joueur a suffisamment de crédits, sinon faux.
     */
    ...
    /**
     * Effectue une opération de crédit/débit sur les crédits du joueur de manière asynchrone.
     *
    ...
    private void creditEconomy(long amount, IFinancialCallback financialCallback) {
        Common.runAsync(() ->  {
            ...
            try {
                // Met à jour le nombre de crédits dans Redis
                this.setHashValue("credit", Long.toString(newAmount));
    ...
    public void expire() {
        try (Jedis jedis = this.api.getBungeeResource()) {
            ...
            jedis.expire(this.jedisAccountKey, 10/*((60 * 60) * 3)*/);
            jedis.expire(this.jedisLogsKey, 10);
        }
    }
    ...
    public boolean loadToJedis(PlayerBean paramPlayerBean) {
        try (Jedis jedis = this.api.getBungeeResource()) {
            ...
            jedis.hsetnx(this.jedisAccountKey, "credit", String.valueOf(paramPlayerBean.getCredit()));
